...
export default function AddProject({saveProject, visibility}){
    //calander view
    ...
    const [project,setProject]=useState({id:Date.now(),title:'',desc:'',startDue:new Date().toLocaleDateString(),endDue:new Date().toLocaleDateString(),tasks:[{id:Math.floor(Math.random() * 100),task:''}]});
    ...
    <ThemeProvider  attribute="class" enableColorScheme={false} defaultTheme="light">
      ...
      <ThemeSelector />
        ...
        <label for="default-input" class={labelStyle}>Project Name</label>
        ...
        <div class="relative">
            ...
            <Datepicker onChange={(date) => {const d = new Date(date).toLocaleDateString(); 
            setProject((prev)=>{return {...prev,startDue:d}}) }} options={options} show={show} setShow={handleClose} name="start" type="text" />
        </div>
        ...
        <div class="relative">
            ...
            <Datepicker onChange={(date) => {const d = new Date(date).toLocaleDateString(); 
            setProject((prev)=>{return {...prev,endDue:d}}) }} show={show2} setShow={handleClose2} name="end" type="text" />
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        {project.tasks.map((task,index)=> {return(<tbody key={task.id}>
